...
def generate_linear_interpolated_path(
    poses: np.ndarray,
    n_interp: int,
    rot_weight: float = 0.1,
):
    """Creates a simple linear interpolated path between keyframe camera poses.

    Linear interpolation is performed in the (position, lookat, up) point space,
    then converted back to camera pose matrices.

    Args:
      poses: (n, 3, 4) array of input pose keyframes.
      n_interp: number of interpolated poses per segment; total returned is
        n_interp * (n - 1).
      rot_weight: relative weighting of rotation/translation in point construction.
    ...
    points = poses_to_points(poses, dist=rot_weight)
    n_key = points.shape[0]

    if n_key < 2 or n_interp <= 0:
        return np.empty((0, 3, 4), dtype=poses.dtype)
    ...
    for i in range(n_key - 1):
        ...
        for j in range(n_interp):
            t = (j + 1) / (n_interp + 1)  # exclude endpoints to avoid duplicates
            linear_points.append(start * (1.0 - t) + end * t)

    linear_points = np.stack(linear_points, axis=0)
    return points_to_poses(linear_points)


def generate_linear_position_only_path(
    poses: np.ndarray,
    n_interp: int,
    focus: np.ndarray | None = None,
):
    """Creates a linear path that only interpolates camera positions.

    Orientation is set to look at a fixed focus point; the up direction is
    taken as the average of input pose up vectors. This yields a trajectory
    with straight positional movement and a consistent look-at behavior, which
    is visually distinct from B-spline or full linear pose interpolation.

    Args:
      poses: (n, 3, 4) array of input pose keyframes.
      n_interp: number of interpolated poses per segment; total returned is
        n_interp * (n - 1). If < 1 or there are fewer than 2 keyframes,
        returns an empty array of shape (0, 3, 4).
      focus: optional world-space point (3,) that cameras look at. If None,
        the focus point is computed via focus_point_fn(poses).
    ...
    # Extract keyframe positions and average up vector
    positions = poses[:, :3, 3]
    avg_up = poses[:, :3, 1].mean(0)
    ...
    for i in range(n_key - 1):
        ...
        for j in range(n_interp):
            t = (j + 1) / (n_interp + 1)  # exclude endpoints to avoid duplicates
            pos = (1.0 - t) * p0 + t * p1
            # Build pose from position, look-at focus, and up direction
            # viewmatrix expects (lookdir, up, position)
            lookdir = pos - focus
            upvec = avg_up
            cam = viewmatrix(lookdir, upvec, pos)
            render_poses.append(cam)
